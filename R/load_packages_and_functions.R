...
neat_names <- c("GNS-I", "GW-SR-I", "GW-VPR-I", "SEED-COVID-ARI-I", "WW-VL")
...
plot_der_prevalence <- function(df = NULL)
{
  ...
  for (var_name in neat_names_no_ww)
  {
    ...
    p <- df  %>%
      ...
      scale_color_manual(
        values = c("#E69F00", colors_prev_plot[, var_name]),
        name = "",
        breaks = c("WW-VL", var_name, paste0(var_name, "_vl")),
        labels = c("WW-VL", var_name, paste0(var_name, " derived prevalence"))
      ) +
      theme_trend() +
...
plot_indicators <- function(df = NULL) {
  ...
  plot_1 <- df %>%
    ...
    geom_point(aes(kalenderwoche, gw_sr, color = "GW-SR-I")) +
    geom_point(aes(kalenderwoche, gw_vpr , color = "GW-VPR-I")) +
    geom_point(aes(kalenderwoche, seed_covidare, color = "SEED-COVID-ARI-I")) +
    ...
    geom_line(aes(kalenderwoche, inzidenz_7_tage_loess, color =   "GNS-I"),
              linewidth = .8) +
    geom_line(aes(kalenderwoche, gw_sr_loess, color = "GW-SR-I"),
              linewidth = .8) +
    geom_line(aes(kalenderwoche, gw_vpr_loess, color = "GW-VPR-I"),
              linewidth = .8) +
    geom_line(aes(kalenderwoche, seed_covidare_loess, color =   "SEED-COVID-ARI-I"),
              linewidth = .8) +
    geom_line(aes(kalenderwoche, (viruslast_loess - alpha) / beta, color = "WW-VL"), linewidth = .8) +
...
plot_changes <- function(df = NULL) {
  ...
  plot_ch <- df %>%
    ...
    geom_line(aes(kalenderwoche, FD, color = Indicator), linewidth = 1) +
